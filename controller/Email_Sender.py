...
def Sender(subject, receiver_email, role, content):
    ...
    if role == "evs":
            html = before_role+"""\
                        ...
                        <a href=""" + content + """><button style="font-size:30px; padding: 10px;border:5px solid aqua;border-radius:25px;">Verify Email</button></a><br>
                """
    elif role == "fg":
            html = before_role+"""\
                        <h3>You are Requested for Resume Builder Password<br>Your Password is : </h3>
                        <button style="font-size:30px; padding: 10px;border:5px solid aqua;border-radius:25px;">""" + content + """</button><br> 
                """
